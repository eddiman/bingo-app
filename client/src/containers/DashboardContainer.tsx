...
`
  ...
  @media ${tokens.constants.device.tablet} {
    ...
    ${(props) =>
      props.state.isWordSelectionContainerOpen
        ? `
top: calc(100% - 50%);
`
        : ""}
...
display: grid;
  ...
  grid-template-columns: repeat(
    ${(props) => (props.state?.gridSize ? props.state.gridSize : 5)},
    minmax(50px, 0.75fr)
  );
...
const DashboardContainer: React.FC = () => {
  ...
  const [isWordSelectionContainerOpen, setIsWordSelectionContainerOpen] =
    useState(false);
  ...
  const [currentlySelectedWord, setCurrentlySelectedWord] =
    useState<IBingoWord>();
  const [currentlySelectedWordBoxString, setCurrentlySelectedWordBoxString] =
    useState<string>();
    ...
    if (bingoWords && newBingoWords) {
      for (let i = 0; i < newBingoWords.length; i++) {
        if (newBingoWords[i].word === currentlySelectedWord?.word) {
          ...
          newBingoWordsOnBoardArray[bingoWordIndex] = {
            word: isSelectingWord ? currentlySelectedWord.word : "",
            isUsed: true,
          };
        } else if (!isSelectingWord) {
          newBingoWords[i] = { word: newBingoWords[i].word, isUsed: false };
          newBingoWordsOnBoardArray[bingoWordIndex] = {
            word: isSelectingWord
              ? newBingoWordsOnBoardArray[bingoWordIndex].word
              : "",
            isUsed: false,
          };
        }
      ...
      let result2 = newBingoWords.filter((person) =>
        newBingoWordsOnBoardArray.every(
          (person2) => !person2.word.includes(person.word)
        )
      );
  ...
  return (
    <>
      ...
      className="Main-Container"
        state={{ direction: "column", wrap: "flex-wrap", childrenFlex: 1 }}
      >
        <SectionContainer state={{wrap: "wrap" }}>
        <h2>{isGameMode ? "Bingo time!" : "Select a word!"}</h2>
        <Button
          onClick={() => {
      ...
      <SectionContainer
      className="tesst"
        state={{ direction: "row", wrap: "flex-wrap", childrenFlex: 1 }}
      >
            ...
            <WordBox
              ...
              onClick={() => {
                isGameMode
                  ? switchWordbox()
                  : setWordInBox(
                      currentlySelectedWord ? currentlySelectedWord.word : "",
                      i
                    );
              }}
        ...
        {isWordSelectionContainerOpen ? (
          <MobileWrapperBg
            onClick={() => {
              setIsWordSelectionContainerOpen(!isWordSelectionContainerOpen);
            }}
          />
        ) : (
          ""
        ...
        <MobileWrapper
          state={{
            isWordSelectionContainerOpen: isWordSelectionContainerOpen,
              ...
              <DecoratedContainer>
                <div className="top-header">
                  {isMobileWindow ? (
                    <CardSheetButton
                      onClick={() =>
                        setIsWordSelectionContainerOpen(
                          !isWordSelectionContainerOpen
                        )
                      }
                      state={{
                      ...
                      children={
                        currentlySelectedWord
                          ? "Valgt ord: " + currentlySelectedWord?.word
                          : data
                          ? " Velg et ord"
                          : "Laster inn..."
                      }
                      ...
                      {bingoWords?.map((wordObj: IBingoWord, i: number) => {
                        return (
                          <WordSelectionModalElement
                            key={i}
                            state={{
                              isUsed: wordObj.isUsed,
                              isDisabled: isGameMode || wordObj.isUsed,
                            }}
                            disabled={isGameMode || wordObj.isUsed}
                            onClick={
                              !wordObj.isUsed
                                ? () => {
                                    elementClicked(wordObj);
                                  }
                                : () => {}
                            }
                          >
